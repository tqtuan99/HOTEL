      ...
      <div class="sidebar-content z-50 shadow-xl h-16 fixed bottom-0 mt-12 lg:h-screen z-10 w-full lg:w-60 left-0" >
         ...
         <div class="lg:mt-12 lg:w-60 lg:fixed lg:left-0 lg:top-0 content-center lg:content-start text-left justify-between">
            <ul class="list-reset lg:pt-7 flex flex-row lg:flex-col py-0 lg:py-4 md:pl-2 text-center lg:text-left overflow-hidden">
               ...
               <li class="flex-1 relative">
                  <a href="?q=employ" class="js-show-data block py-1 md:py-3 lg:pl-3 align-middle text-white no-underline">
               ...
               <li class="flex-1 relative">
                  <a href="?q=statistical" class="js-show-data block py-1 md:py-3 lg:pl-3 align-middle text-white no-underline">
                  ...
                  <div class="flex flex-row items-center">
                     ...
                     <div class="js-show-statistical flex-1 text-right md:text-center">
                        ...
                        <h3 class="font-bold text-2xl">
                           ...
                           $queryTotal = 'SELECT sum(ROUND(TIME_TO_SEC(timediff(ngaythanhtoan,ngaytao))*(phong.dongia/3600/24),2)) as tong 
                                          FROM `hoadon`, `phong` 
                                          WHERE hoadon.idphong = phong.idphong and hoadon.trangthai =1';
                     ...
                     <div class="js-show-statistical hidden flex-1 text-right md:text-center">
                        ...
                        <h3 class="font-bold text-2xl">
                           ...
                           $queryTotal = 'SELECT sum(ROUND(TIME_TO_SEC(timediff(ngaythanhtoan,ngaytao))*(phong.dongia/3600/24),2)) as tong 
                                          FROM `hoadon`, `phong` 
                                          WHERE hoadon.idphong = phong.idphong and datediff(CURDATE(),ngaythanhtoan) < 30 and hoadon.trangthai =1';
                           $result = $conn->query($queryTotal);
                           if ($result) {
                              $row = $result->fetch_assoc();
                              echo number_format($row['tong']);
                     ...
                     <div class="js-show-statistical hidden flex-1 text-right md:text-center">
                        ...
                        <h3 class="font-bold text-2xl">
                           ...
                           $queryTotal = 'SELECT sum(ROUND(TIME_TO_SEC(timediff(ngaythanhtoan,ngaytao))*(phong.dongia/3600/24),2)) as tong 
                                          FROM `hoadon`, `phong` 
                                          WHERE hoadon.idphong = phong.idphong and datediff(CURDATE(),ngaythanhtoan) <= 7 and hoadon.trangthai =1';
                           $result = $conn->query($queryTotal);
                           if ($result) {
                              $row = $result->fetch_assoc();
                              echo number_format($row['tong']);
                     ...
                     <div class="js-show-statistical hidden flex-1 text-right md:text-center">
                        ...
                        <h3 class="font-bold text-2xl">
                           ...
                           $queryTotal = 'SELECT sum(ROUND(TIME_TO_SEC(timediff(ngaythanhtoan,ngaytao))*(phong.dongia/3600/24),2)) as tong 
                                          FROM `hoadon`, `phong` 
                                          WHERE hoadon.idphong = phong.idphong and datediff(CURDATE(),ngaythanhtoan) <= 1 and hoadon.trangthai =1';
                           $result = $conn->query($queryTotal);
                           if ($result) {
                              $row = $result->fetch_assoc();
                              echo number_format($row['tong']);
